...
- **Supersets / grouped exercises.** Designed in detail but deferred. Would let users group N exercises that are performed back-to-back in rounds (chest+back supersets, biceps+triceps, etc.). Data model: `group_id` UUID on history rows + `group_assignments` jsonb on plans. UX: visual grouping in plan editor (drag-and-drop), ad-hoc grouping mid-workout via "+ Group" multi-select, loose enforcement (visual cue only, not enforced round order), grouped display in past sessions. Stats unchanged. See conversation history for full design. Build when real usage demonstrates demand — at least one of [Mac, testers] regularly using supersets in their training and feeling the lack.
